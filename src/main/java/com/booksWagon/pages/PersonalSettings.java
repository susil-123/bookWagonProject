...
//		select
    ...
    public ArrayList<String> returnColumn(int columnNumber) throws IOException {
		...
		for(int row=1;row<rowCount;row++) {
			String tc = objExcel.getCellData(excelPath,"personalSettingsSheet",row,columnNumber);
			column.add(tc);
		}
		return column;
    }
...
//	public ArrayList<String> fillDetailsExcel() throws IOException, InterruptedException {
		...
		String actualMobileBeforeClearing = null;
		for(int row=1;row<rowCount;row++) {
			actualEmailBeforeClearing = driver.findElement(inpEmail).getAttribute("value");
			actualMobileBeforeClearing = driver.findElement(inpMobileNumber).getAttribute("value");
...
//	        btn.click();
	        ...
	        if(!(actualEmailBeforeClearing.equals(email)) || ! (actualMobileBeforeClearing.equals(mobileNumber)) ) {
	        	try {
	        		...
	        		testCase.add("fail");
//	        		objExcel.setCellData(excelPath,"personalSettingsSheet",row,12,testCase.get(row-1));
	        		System.out.println("failed using timeout exception");
...
//		driver.navigate().back();
		...
		writeActualTestCaseOnExcel(testCase);
//		return testCase;
	}
